...
fn test_auto_hv_single_point_valid_reference() {
    ...
    let hv = hypervolume_2d_auto(&front, &reference);
...
fn test_auto_hv_ordering_invariant() {
    ...
    let hv1 = hypervolume_2d_auto(&front1, &reference);
    let hv2 = hypervolume_2d_auto(&front2, &reference);
...
fn test_auto_hv_empty_front_returns_zero() {
    let hv = hypervolume_2d_auto(&[], &[5.0, 5.0]);
    assert_eq!(hv, 0.0);
}
...
fn test_auto_hv_monotonicity_when_reference_expands() {
    ...
    let hv_small = hypervolume_2d_auto(&front_small, &reference);
    let hv_large = hypervolume_2d_auto(&front_large, &reference);
